...
<!--HTML and CSS code to style the site-->
	...
	if(isset($_POST["submit"])){
		if(isset($_POST["name"]) && $_POST["name"]!== ''){
			$query="INSERT INTO Owner(SignatoryID,SignatoryName) VALUES('','".$_POST["name"]."')";
			$result=$mysqli->query($query);
			if($result){
				echo "Add Successful";
				redirect_to('addOwner.php');
			}
			else{
				echo "Add Failed";
			}
		...
		<div class="row">
			<label for="left-label" class="left inline">
				...
				<h2>Add New Account Owner</h2>

				<form method="post" action="addOwner.php">
					<p>Enter The Name of the New Account Owner: <input type ="text" name="name" /></p>

					<input type="submit" class="button large round" name="submit" value="Add Owner"></input>
